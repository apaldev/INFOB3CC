...
casIOVectorFloat :: S.IOVector Float -> Int -> Float -> Float -> IO (Bool, Float)
casIOVectorFloat (S.MVector _ (ForeignPtr addr _)) idx (F# check) (F# repl) = do
  let size = 4
...
atomicModifyIOVectorFloat :: S.IOVector Float -> Int -> (Float -> (Float, b)) -> IO b
atomicModifyIOVectorFloat vector idx f = do
  value <- S.read vector idx
  go value
  where
    go value = do
      let (new, b) = f value
